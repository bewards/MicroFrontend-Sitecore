...
To pull new item updates from `dev` endpoint to local file system, run the following commands:
1. `dotnet sitecore cloud login`
1. `dotnet sitecore cloud environment connect --environment-id <environment-id>`
1. `dotnet sitecore index list --environment-name dev`
1. `dotnet sitecore serialization pull -n dev`